class NumMatrix:
    ...
    def __init__(self, matrix: List[List[int]]):
        ...
        for r in range(rows): 
            for c in range(cols):
                left = self.pre[r][c - 1] if c > 0 else 0 
                above = self.pre[r - 1][c] if r > 0 else 0 
                diag = self.pre[r - 1][c - 1] if r > 0 and c > 0 else 0 
                self.pre[r][c] = matrix[r][c] + left + above - diag
    ...
    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        ...
        diag = self.pre[row1 - 1][col1 - 1] if row1 > 0 and col1 > 0 else 0
